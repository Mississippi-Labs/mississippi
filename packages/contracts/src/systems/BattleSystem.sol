...
contract BattleSystem is System {
    ...
    function confirmBattle(
        ...
        require(
            block.timestamp - battle.timestamp < BattleConfig.getMaxTimeLimit(BATTLE_CONFIG_KEY),
            "Battle is timeout"
        );
        // 战斗是否已经选择buff
    ...
    function revealWinner(
        ...
        if (
            battle.attackerAction == bytes32("attack") &&
            battle.defenderAction == bytes32("attack")
        ) {
            ...
            Buff defenderBuff = Buff(battle.defenderArg);
            // 任意攻击buff都强于None
            uint256 attackerAttackPower = BattleUtils.getAttackPower(
                attackerBuff,
                defenderBuff,
                attackerFirepower
            );
            uint256 defenderAttackPower = BattleUtils.getAttackPower(
                defenderBuff,
                attackerBuff,
                defenderFirepower
            );

            BattleList.setAttackerHP(_battleId, BattleUtils.getAttackResult(
                battle.attackerHP,
                defenderAttackPower
            ));
        ...
        if (
            battle.attackerAction == bytes32("escape") &&
            battle.defenderAction == bytes32("escape")
        ) {
        ...
        if (
            ...
            if (
                ...
                BattleUtils.compareBuff(attackerBuff, defenderBuff) == 2
            ) {
                // 逃跑成功
        ...
        if (
            ...
            if (
                attackerBuff == defenderBuff ||
                BattleUtils.compareBuff(defenderBuff, attackerBuff) == 2
            ) {
                // 逃跑成功
                ...
                //     attackerAttackPower
                // ));
                battle.defenderHP = BattleUtils.getAttackResult(
                    battle.defenderHP,
                    attackerAttackPower
                );
